...
def detect_satellite_points(image_path, model_path='params/satellite_net_best.pth', image_size=256):
    """
    检测卫星图像的关键点
    """
    # 设备设置
    ...
    # 加载模型
    net = SatelliteNet(num_points=10, pretrained=False).to(device)
    ...
    # 保持宽高比调整大小
    scale = min(image_size/original_size[0], image_size/original_size[1])
    ...
    # 转换为模型输入
    img_tensor = tf(canvas).unsqueeze(0).to(device)
    ...
    # 后处理：反归一化并调整到原始图像坐标
    points = predictions[0].cpu().numpy() * image_size
    points = points.reshape(-1, 2)  # 转换为10x2的数组
    
    # 调整坐标到原始图像尺寸
    adjusted_points = []
    for x, y in points:
        # 调整到resized图像坐标
        x_adj = max(0, min(x - x_offset, new_size[0]))
        y_adj = max(0, min(y - y_offset, new_size[1]))
        
        # 调整到原始图像坐标
        x_orig = x_adj / scale
        y_orig = y_adj / scale
        
        adjusted_points.append((int(x_orig), int(y_orig)))
    
    return adjusted_points, img

def visualize_detection(image_path, output_path=None):
    """
    可视化检测结果
    """
    points, original_img = detect_satellite_points(image_path)
    ...
    for i, (x, y) in enumerate(points):
        ...
        # 添加编号
        draw.text((x+5, y-5), str(i+1), fill=color)
    ...
    original_img.show()
    return points, original_img

if __name__ == '__main__':
    # 测试单张图像
    image_path = 'test_image/your_satellite_image.jpg'  # 替换为您的测试图像
    points, result_img = visualize_detection(image_path, 'detection_result.jpg')
